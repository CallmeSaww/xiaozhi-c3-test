...
void OledDisplay::SetupUI_128x64() {
    ...
    dasaimochi_anim_.Init();
    emotion_img_ = lv_image_create(content_left_);
    const lv_img_dsc_t* init_dsc_64 = dasaimochi_anim_.GetNextFrame();
    if (init_dsc_64 != nullptr) {
        lv_image_set_src(emotion_img_, init_dsc_64);
    }
    lv_obj_center(emotion_img_);
    lv_obj_set_style_pad_top(emotion_img_, 8, 0);
...
void OledDisplay::SetupUI_128x32() {
    ...
    /* Emotion image on the left side */
    content_ = lv_obj_create(container_);
    ...
    const lv_img_dsc_t* init_dsc_32 = dasaimochi_anim_.GetNextFrame();
    if (init_dsc_32 != nullptr) {
        lv_image_set_src(emotion_img_, init_dsc_32);
    }
    lv_obj_center(emotion_img_);
